...
export type ModelWithVendor = { name: string; tier: string; vendor: string };

type Props = {
    tier: string;
    models: ModelWithVendor[];
};

const vendorLogoUrl = (vendor: string): string => {
    ...
    return map[vendor] ?? 'https://cdn.jsdelivr.net/gh/simple-icons/simple-icons/icons/artificialintelligence.svg';
};

const getTierInfo = (tier: string) => {
    return { name: `Tier ${tier}`, description: '', icon: '🤖' };
};

const getVendorName = (vendor: string): string => {
    ...
    return vendorNames[vendor] ?? vendor.toUpperCase();
};
...
export const ClusterCard: React.FC<Props> = ({ tier, models }) => {
    const tierInfo = getTierInfo(tier);

    // Group models by vendor for better organization
    const modelsByVendor = models.reduce((acc, model) => {
        const vendor = model.vendor;
        if (!acc[vendor]) acc[vendor] = [];
        acc[vendor].push(model);
        return acc;
    }, {} as Record<string, ModelWithVendor[]>);

    return (
        <section className="cluster" style={{ animationDelay: `${parseInt(tier) * 0.1}s` }}>
            ...
            <div className="models">
                {models.map((model, index) => (
                    <div
                        key={model.name}
                        className="model"
                        style={{ animationDelay: `${(parseInt(tier) * 0.1) + (index * 0.05)}s` }}
                        title={`${model.name} by ${getVendorName(model.vendor)}`}
                    >
                        <div className="model-logo">
                            {model.vendor === 'deepseek' ? (
                                ...
                                <img
                                    src={vendorLogoUrl(model.vendor)}
                        ...
                        <div className="model-info">
                            ...
                            <div className="model-vendor">{getVendorName(model.vendor)}</div>
                        </div>
                    </div>
                ))}
            </div>
